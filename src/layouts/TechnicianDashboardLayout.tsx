...
const links = [
  { name: "Overview", icon: Activity, to: "/dashboard/technician" },
  { name: "Job Board", icon: Briefcase, to: "/dashboard/technician/job-board" },
  { name: "My Jobs", icon: ClipboardList, to: "/dashboard/technician/my-jobs" },
  { name: "Earnings", icon: DollarSign, to: "/dashboard/technician/earnings" },
  { name: "Messages", icon: MessageSquare, to: "/dashboard/technician/messages" },
  { name: "Profile", icon: User, to: "/dashboard/technician/profile" },
] as const;
...
function SidebarLink({ item, selected, open }: SidebarLinkProps) {
  ...
  return (
    <NavLink to={item.to} end={item.to === "/dashboard/technician"}
      className={({ isActive }) => [
        ...
        <div className="flex items-center gap-3">
          ...
          {open && (
            <div className="transition-opacity duration-200">
              ...
              <span className="block text-xs text-gray-500">Technician</span>
            </div>
          )}
        </div>
  ...
  return (
    <button onClick={()=>setOpen(!open)} className="absolute bottom-0 left-0 right-0 border-t border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
      <div className="flex items-center p-3">
        <div className="grid size-10 place-content-center"><ChevronsRight className={`h-4 w-4 transition-transform ${open?"rotate-180":""}`}/></div>
